...
exports.getOverview = async (req, res) => {
    try {
        ...
        attDocs.forEach((doc) => {
            ...
            visits.forEach((v) => {
                if (v.checkIn && v.checkOut) {
                    ...
                    if (!latestCheckOut || end > latestCheckOut) {
                        latestCheckOut = end;
                    }
                }
            });
        ...
        bookedSlots.forEach((slot) => {
            ...
            const studentId =
                student && student._id ? String(student._id) : student ? String(student) : null;
...
exports.downloadCenterCsv = async (req, res) => {
    try {
        ...
        const totalVisits = sortedSlots.length;
        const avgVisitMinutes =
            totalVisits > 0 ? Number((totalMinutes / totalVisits).toFixed(1)) : 0;
...
exports.downloadTutorCsv = async (req, res) => {
    try {
        ...
        if (!slots.length) {
            // Still produce a small CSV with just summary
            const emptyRows = [
                ["Tutor Report"],
